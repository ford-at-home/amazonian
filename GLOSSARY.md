...
The named failure mode `14-lifecycle-navigator` exists to catch. The navigator reports a clean state and recommends an advanced-phase skill (e.g., `launch-readiness-review`, `tenets-review`, `ambitious-goal-grading`); on the surface, the chain looks healthy because the manifest is populated and the recommended skill's invocation is technically valid. The structural failure underneath is that the upstream artifacts the recommended skill assumes — PRFAQ for LRR, tenets for tenets-review, period goals and prior PRFAQ for grading — are themselves `evidence: assumption` from the bootstrap, or are missing entirely. The recommended interrogative skill then stress-tests laundered brownfield state instead of governance-produced artifacts, and a clean report from that skill is taken as validation when nothing valid has actually been tested. The skill's second-axis check is the requirement that every recommendation trace its target skill's `required_artifacts` to manifest fields, verify those fields carry `evidence` in `{fact, inference}` with resolving sources, and demote any recommendation against `assumption`- or `open_question`-tagged dependencies to `upstream_remediation` — routing the operator to the constructive skill that produces the missing field rather than the interrogative skill that would have stress-tested its laundered version. Analogous in structure to the other named failure modes in this index.

## Brownfield laundering

The informal named risk `00-repo-state-import` is designed to prevent. The lifted `governance.yaml` manifest's surface signal — every field populated, schema valid, downstream skills runnable — hides the structural failure that the populated state never actually passed any governance gate. Downstream skills, reading the manifest, treat lifted assumptions as if they were the output of the constructive skills that should have produced them. The mitigation is that every claim-bearing manifest field carries an evidence tag from `{fact, assumption, inference, open_question}`, `fact` requires a non-null source, and `assumption`-tagged fields propagate as routing constraints in `14-lifecycle-navigator` (via the `phase laundering` second-axis check above). This risk is declared informally on a constructive skill rather than formally in the cross-reference index below — the formal pattern is reserved for interrogative skills that gate *ongoing* validity over time, per [`SKILL_DESIGN_PATTERN.md`](SKILL_DESIGN_PATTERN.md#named-failure-modes). Brownfield laundering is a *bootstrap-time* risk: it is real, but its character is mitigated through artifact discipline at production, not through a separate stress-test pass over time.
...
| [`lifecycle-navigator`](#phase-laundering) | **phase laundering** | manifest populated, advanced-phase skill recommendable | trace recommended skill's `required_artifacts` to manifest fields; demote to upstream remediation on assumption-tagged dependencies |
...
Constructive skills may declare *informal* named risks (e.g., [`brownfield laundering`](#brownfield-laundering) on `00-repo-state-import`); those are documented inline above but do not appear in this formal cross-reference, which is reserved for the interrogative-skill-over-time pattern.
